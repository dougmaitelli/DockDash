...
test("service changelog, files, logs and terminal", async ({ page }) => {
  ...
  await screenshot(page, "service-changelog.png");
  await page.getByRole("button", { name: "Files", exact: true }).click();
  await expect(page.getByText("traefik", { exact: true }).last()).toBeVisible();
  await screenshot(page, "service-files.png");
  await page.getByRole("button", { name: "Logs", exact: true }).click();
  await expect(page.getByText(/GET \/healthz 200 1ms/)).toBeVisible();
  await screenshot(page, "service-logs.png");
  await page.getByRole("button", { name: "Terminal", exact: true }).click();
  await expect(page.getByText("Connected", { exact: true })).toBeVisible();
  await expect(page.locator(".xterm-rows")).toContainText("root@traefik");
  ...
  await screenshot(page, "service-terminal.png");
});
...
test("discovery imports a service through the real API", async ({ page, request }) => {
  ...
  await expect(page.getByText("Found 7 containers")).toBeVisible();
...
test("dashboard error and retry", async ({ page }) => {
  // Deliberate transport failure; all other tests use the real API.
