...
async function fetchToken(registry: string, repository: string): Promise<string | null> {
  try {
 ...
 */
export async function getManifestDigest(ref: ImageRef): Promise<string | null> {
  try {
    const token = await fetchToken(ref.registry, ref.repository);
 ...
 */
async function getDockerHubTags(repository: string, prefix: string): Promise<string[]> {
  ...
  const firstTags = (firstResp.data?.results as Array<{ name: string }>)?.map((r) => r.name) ?? [];
 ...
 */
export async function getRepositoryTags(ref: ImageRef, prefix = ""): Promise<string[]> {
  try {
    if (ref.registry === "registry-1.docker.io") {
      return await getDockerHubTags(ref.repository, prefix);
    }
